...
class TestMemory(unittest.TestCase):
    def setUp(self) -> None:
        ...
        os.environ["KAAL_SESSIONS_DIR"] = self._sessions_tmp.name
    ...
    def test_store_dir_override(self):
        self.assertEqual(get_store_dir(), Path(os.environ["KAAL_SESSIONS_DIR"]))
...
class TestPrompts(unittest.TestCase):
    ...
    def test_project_context_includes_structure_cache(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            cwd.mkdir()
            (cwd / ".kaal").mkdir()
            (cwd / ".kaal" / "STRUCTURE.md").write_text(
                "# Project Structure\nRoot: x\n## Tree\n└── README.md (5 B)\n",
                encoding="utf-8",
            )
            ctx = build_project_context(cwd)
    ...
    def test_build_system_prompt(self):
        ...
        project = "Date: 2026-08-02\nCWD: /tmp/x"
        prompt = build_system_prompt(digest, project)
        self.assertIn("kaal — DeepSeek V4 Flash harness agent", prompt)
        self.assertIn(
