...
class CountingToolRegistry(ToolRegistry):
    ...
    def __init__(self, project_dir):
        self._cache_path = Path(project_dir) / ".kaal" / "tool-cache.json"
        self.read_calls = 0
...
class TestAgentLoop(unittest.TestCase):
    ...
    def test_structure_cache_refreshed_after_tools(self):
        ...
        doc = (self.tempdir / ".kaal" / "STRUCTURE.md").read_text(encoding="utf-8")
    ...
    def test_tool_cache_disabled_without_signature(self):
        ...
        self.assertFalse((self.tempdir / ".kaal" / "tool-cache.json").exists())

    # -- verify hooks --------------------------------------------------------

    def _write_hooks(self, verify_cmd):
        hooks = self.tempdir / ".kaal" / "hooks.json"
