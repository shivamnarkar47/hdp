...
class AgentLoop:
    ...
    def __init__(
        ...
        # Verify hook command from .hdp/hooks.json, read ONCE at run() start
    ...
    def _load_verify_cmd(self) -> None:
        """Read .hdp/hooks.json once into self._verify_cmd (None = off).
        ...
        if not self.enable_verify:
            return
        hooks_path = self.tools.project_dir / ".hdp" / "hooks.json"
        try:
    ...
    def _spawn(self, task: str, dir: str | None, max_steps: int, timeout: int) -> str:
        ...
        ``hdp sessions list``), allow_dangerous=False, enable_verify=False,
